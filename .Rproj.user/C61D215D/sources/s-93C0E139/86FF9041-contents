...
for (sub in unique(d$Participant_ID)) {
  cat("\n")
  cat(sub)
  q = filter(d, Participant_ID == sub)
  cat("\n")
  cat(unique(q$Treatment))
}
...
for (sub in unique(d$Participant)) {
    ...
    for (tr in unique(d1$Treatment)) {
        ...
        p = ggplot(d2, aes(x = Treatment_Time, y = BlinkStatus)) + geom_line() + theme_bw() + xlab("") + ylab(tr) + scale_y_continuous(breaks = c(0,1)) + theme(axis.title.y = element_text(angle = 0, face = "bold", size = 18))
