...
d = filter(d, newPR == 1)
dirn = paste0("Plots/1Hz_Timeseries_AllEmo_Subject+CumulativeJudges_",Sys.Date())
dir.create(dirn)
dirni = paste0("Plots/1Hz_Timeseries_AllEmo_Subject+CumulativeJudges_",Sys.Date(),"/NI")
dir.create(dirni)
diri = paste0("Plots/1Hz_Timeseries_AllEmo_Subject+CumulativeJudges_",Sys.Date(),"/I")
dir.create(diri)
...
for (sub in i) {
  ...
  j = jdraw_area_plot(d1, sub, "PR", "bar")
  
  x = ggarrange(f,j, ncol = 1, nrow = 2, common.legend = T)
  
  ggsave(x, filename = paste0(diri,"/",sub,".pdf"))
}


# Not Informed subjects
for (sub in ni) {
  print(sub)
  d1 = filter(d, Participant_ID == sub)
  d1 = d1[,c(68,19, scol, jcol, 72)]
